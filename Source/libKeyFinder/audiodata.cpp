...
namespace KeyFinder {
  ...
  void AudioData::setSampleByFrame(unsigned int frame, unsigned int channel, double value) 
  {
	...
	//DBG("SetSmpByFrame-- FrameCount: " + String(frameCount) + " frame:" + String(frame) + " Channel: " + String(channel) + " Value: " + String(value));

    if (frame >= frameCount) {
      std::ostringstream ss;
      ss << "Cannot set out-of-bounds frame (" << frame << "/" << frameCount << ")";
      throw Exception(ss.str().c_str());
    }
    if (channel >= channels) {
      std::ostringstream ss;
  ...
  void AudioData::reduceToMono() {
	...
	std::deque<double>::const_iterator sampleEnd = samples.end();
    //while (readAt < samples.end()) 
	while (readAt < sampleEnd)
	{
      double sum = 0.0;
  ...
  void AudioData::downsample(unsigned int factor, bool shortcut) {
	...
	// Prevents std::advance out of iterator range problems
